...
export default function App() {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    { headerName : 'bbb', field : 'description', sortable:true, filter:true },
    { headerName : 'ccc', field : 'location', sortable:true, filter:true },
  ]);
  ...
  const onSelectedRows= () =>{
    ...
    const selectedRowsData = selectedRows
      .map(rows => `${rows.name} ${rows.location}`).join('. ');      
